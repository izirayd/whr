...
function ratingEloToNaturalR(ratingElo) {
...
function normalizedModeSql(alias) {
    return `
CASE
...
export class WhrRepository {
    ...
    listPlayers(search, page, pageSize, scope, latestRunOnly) {
...
`)
                .get(latestRunId, scope, queryText, like);
            const rows = this.db
                .prepare(`
SELECT
  p.id AS id,
...
LIMIT ? OFFSET ?;
`)
                .all(latestRunId, scope, queryText, like, pageSize, offset);
            return {
                latestRunId,
                total: total.total,
                items: rows
...
`)
    ...
    getPlayerProfile(playerId, historyLimit, matchesLimit, scope) {
        const latestRunId = this.getLatestCompletedRunId();
...
`)
        ...
        const scopeTotalPlayers = latestRunId !== null ? this.getScopeTotalPlayers_(latestRunId, scope) : undefined;
        const selectedScopeRating = ratings[scope];
        const scopeRankPosition = latestRunId !== null && scopeTotalPlayers !== undefined && scopeTotalPlayers > 0
            ? this.getSnapshotRankPosition_(latestRunId, scope, playerId, selectedScopeRating.ratingElo)
            : undefined;
...
LIMIT ?;
`)
            .all(playerId, scope, matchesLimit);
        const matchRatingsById = this.getMatchRatingsByPlayer_(matchRows.map((row) => row.id), scope, latestRunId);
        const rankByPlayerAndRatingCache = new Map();
        const resolveRankForPlayer = (targetPlayerId, rating) => {
            if (latestRunId === null ||
                rating === undefined ||
        ...
        const recentMatches = matchRows.map((row) => {
            ...
            const rankPositionsByPlayer = new Map();
            if (ratingsByPlayer) {
            ...
            const sides = this.getMatchSides(row.id, ratingsByPlayer, rankPositionsByPlayer.size > 0 ? rankPositionsByPlayer : undefined);
            return {
                id: row.id,
                mode: row.mode,
                ...
                playerRatingDelta: selectedPlayerRating !== undefined
    ...
    listMatches(filters) {
        ...
        if (filters.latestRunOnly) {
            ...
            where.push("EXISTS (SELECT 1 FROM player_rating_history h WHERE h.run_id = ? AND h.match_id = m.id)");
        ...
        if (filters.playerId !== undefined) {
            where.push("EXISTS (SELECT 1 FROM match_sides ms2 JOIN side_players sp2 ON sp2.side_id = ms2.id WHERE ms2.match_id = m.id AND sp2.player_id = ?)");
...
LIMIT ? OFFSET ?;
`);
        const rows = pageStmt.all(...params, filters.pageSize, offset);
        const matchIdsByMode = {
            duel: [],
            team_small: [],
            team_large: [],
            ffa: []
...
WHERE id IN (?, ?);
`)
            .all(input.playerAId, input.playerBId);
        if (playerRows.length !== 2) {
            const foundIds = new Set(playerRows.map((row) => row.id));
            const missing = [];
            if (!foundIds.has(input.playerAId))
                missing.push(input.playerAId);
            if (!foundIds.has(input.playerBId))
                missing.push(input.playerBId);
            throw new Error(`Players not found: ${missing.join(", ")}`);
        }
        const playerHandlesById = new Map();
...
`);
    ...
    createRecalcJob() {
        ...
        if (!job)
            throw new Error("Failed to create recalc job");
        return job;
    }
    getRecalcJob(jobId) {
        const row = this.db
...
`)
    ...
    markRecalcJobFailed(jobId, message) {
        this.db
            .prepare(`
UPDATE recalc_jobs
SET status = 'failed',
...
`)
    ...
    getMatchSides(matchId, ratingsByPlayer, rankPositionsByPlayer) {
        const rows = this.db
            .prepare(`
SELECT
...
`)
    ...
    computeWinProbabilities_(sides, ratingsByPlayer) {
        if (sides.length === 0)
            return undefined;
        if (sides.length === 1)
            return [1.0];
    ...
    getMatchRatingsByPlayer_(matchIds, scope, latestRunId) {
...
ORDER BY h.id DESC;
`)
            .all(...matchIds, scope, ...(latestRunId !== null ? [latestRunId] : []));
    ...
    getScopeTotalPlayers_(runId, scope) {
        const row = this.db
            .prepare(`
SELECT COUNT(1) AS total
FROM rating_snapshots
...
`)
    ...
    getSnapshotRankPosition_(runId, scope, playerId, ratingElo) {
        const row = this.db
            .prepare(`
...
`)
            .get(runId, scope, ratingElo, ratingElo, playerId);
        return row.better + 1;
    }
    getNextPlayedAt_() {
        const row = this.db
...
`);
        ...
        if (!this.tableHasColumn_("player_rating_history", "rating_scope")) {
            this.db.exec("ALTER TABLE player_rating_history ADD COLUMN rating_scope TEXT NOT NULL DEFAULT 'duel';");
        }
...
`);
    ...
    tableHasColumn_(tableName, columnName) {
        const rows = this.db.prepare(`PRAGMA table_info(${tableName});`).all();
        return rows.some((row) => row.name === columnName);
    }
    hasEmptyRatingScopeValues_(tableName) {
        const row = this.db
            .prepare(`
SELECT EXISTS(
  SELECT 1
...
`)
    ...
    queryPlayerHistory_(playerId, scope, historyLimit, latestRunId) {
        if (latestRunId !== null) {
            return this.db
                .prepare(`
SELECT
...
LIMIT ?;
`)
                .all(playerId, scope, latestRunId, historyLimit);
        }
        return this.db
            .prepare(`
SELECT
...
LIMIT ?;
`)
            .all(playerId, scope, historyLimit);
    }
}
