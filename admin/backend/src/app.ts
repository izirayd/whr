...
interface ApiServices {
  repository: WhrRepository;
  recalcRunner: RecalcRunner;
}
...
function parsePositiveInt(raw: unknown, fallback: number, maxValue: number): number {
...
function parseScope(raw: unknown): RatingScope | undefined {
  if (raw === "duel" || raw === "team_small" || raw === "team_large" || raw === "ffa") return raw;
  return undefined;
}

function parseMode(raw: unknown): MatchMode | undefined {
  return parseScope(raw);
}

function parseOptionalNonNegativeNumber(raw: unknown, fieldName: string): number | undefined {
  if (raw === undefined || raw === null || raw === "") return undefined;
  const value = Number(raw);
...
function parseOptionalPositiveNumber(raw: unknown, fieldName: string): number | undefined {
  if (raw === undefined || raw === null || raw === "") return undefined;
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
...
function parseOptionalPositiveInteger(raw: unknown, fieldName: string): number | undefined {
  if (raw === undefined || raw === null || raw === "") return undefined;
  const value = Number(raw);
...
function parseRequiredPositiveInteger(raw: unknown, fieldName: string): number {
...
function parseOptionalNonNegativeInteger(raw: unknown, fieldName: string): number | undefined {
  if (raw === undefined || raw === null || raw === "") return undefined;
  const value = Number(raw);
  if (!Number.isFinite(value) || value < 0 || !Number.isInteger(value)) {
...
function parseRecalcLaunchOptions(raw: unknown): RecalcLaunchOptions {
  ...
  return {
    ...
    finalOptimizeIterations: parseOptionalNonNegativeInteger(
      payload.finalOptimizeIterations,
...
function parseCreateDuelPayload(raw: unknown): {
  duelInput: CreateDuelMatchInput;
...
export function createApiApp(services: ApiServices): express.Express {
  const app = express();
  app.use(cors());
  ...
  app.post("/api/admin/duels", (req: Request, res: Response) => {
    let payload: {
      duelInput: CreateDuelMatchInput;
      recalcOptions: RecalcLaunchOptions;
    };
  ...
  app.post("/api/admin/recalculate", (req: Request, res: Response) => {
  ...
  app.get("/api/admin/recalculate/:jobId", (req: Request, res: Response) => {
    try {
